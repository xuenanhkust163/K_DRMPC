...
def run_single_method(method_name, track, controller, dist_gen, max_steps):
    ...
    返回:
        result: 仿真结果对象，包含：
            - states: 状态轨迹列表 [[px, py, v, psi, omega], ...]
...
def run_all_methods_on_track(track, model, D, norm_params, dist_gen,
    ...
    # 1. LMPC（线性MPC）
    # ================================================================
    # 线性MPC使用在工作点线性化的模型，计算速度快但精度较低
    # 适合小偏差跟踪，但在大曲率赛道可能表现不佳
    print(f"\n{'='*60}")  # 打印分隔线（60个等号），区分不同方法的仿真
    print(f"在 {track.__class__.__name__} 上运行LMPC")  # 显示当前赛道和方法名
    print(f"{'='*60}")
    lmpc = LMPCController()  # 创建线性MPC控制器实例（不需要模型参数）
    # 运行LMPC仿真并保存结果到"LMPC_{赛道名}.pkl"
    results['LMPC'] = run_single_method('LMPC', track, lmpc, dist_gen, max_steps)

    # ================================================================
    # 2. NMPC（非线性MPC）
    # ================================================================
    # 非线性MPC使用完整的非线性自行车模型，精度最高
    # 但每次求解需要解非线性优化问题，计算量大
    print(f"\n{'='*60}")
    print(f"在 {track.__class__.__name__} 上运行NMPC")
    print(f"{'='*60}")
    nmpc = NMPCController()  # 创建非线性MPC控制器实例
    # 运行NMPC仿真并保存结果到"NMPC_{赛道名}.pkl"
    results['NMPC'] = run_single_method('NMPC', track, nmpc, dist_gen, max_steps)

    # ================================================================
    # 3. K-MPC（Koopman MPC）
    # ================================================================
    # K-MPC使用Deep Koopman网络将非线性系统提升到线性空间
    # 在Koopman空间中使用线性MPC，兼顾速度和精度
    print(f"\n{'='*60}")
    print(f"在 {track.__class__.__name__} 上运行K-MPC")
    print(f"{'='*60}")
    # 创建Koopman MPC控制器，需要传入模型、投影矩阵和归一化参数
    # model: Deep Koopman网络（编码器和解码器）
    # D: 投影矩阵（干扰映射）
    # norm_params: 归一化参数（数据标准化）
    kmpc = KMPCController(model, D, norm_params)
    # 运行K-MPC仿真并保存结果到"K-MPC_{赛道名}.pkl"
    results['K-MPC'] = run_single_method('K-MPC', track, kmpc, dist_gen, max_steps)
...
def main():
    ...
    # ================================================================
    print("\n--- 创建赛道 ---")
    # 创建卢赛尔赛道（Lusail Track）
    # 这是F1卡塔尔大奖赛的真实赛道，用于主要实验
    lusail = LusailTrack()
    ...
    print("#" * 60)
    # 运行4种方法，返回结果字典
    # 结果保存到：LMPC_LusailTrack.pkl, NMPC_LusailTrack.pkl,
    #           K-MPC_LusailTrack.pkl, K-DRMPC_LusailTrack.pkl
    lusail_results = run_all_methods_on_track(
    ...
    # 步骤6：鲁棒性分析（论文表11）
    # ================================================================
    # 测试K-DRMPC在不同干扰水平（sigma）下的性能
    # sigma值越大，干扰越强，测试算法的鲁棒性
    print("\n" + "#" * 60)
    print("# 鲁棒性分析（表11）")
    print("#" * 60)
    # 在卢赛尔赛道上运行，测试SIGMA_VALUES中的所有sigma值
    # 结果保存到：robustness_sigma_0.01.pkl, robustness_sigma_0.05.pkl, ...
    robustness = run_robustness_analysis(
        lusail, model, D, norm_params, max_steps=500)

    # ================================================================
    # 步骤7：敏感性分析 - theta（论文表14）
    # ================================================================
    # 测试K-DRMPC在不同Wasserstein半径（theta）下的性能
    # theta控制分布鲁棒优化中模糊集的大小
    print("\n" + "#" * 60)
    print("# 敏感性分析 - THETA（表14）")
    print("#" * 60)
    # 在卢赛尔赛道上运行，测试THETA_VALUES中的所有theta值
    # 结果保存到：sensitivity_theta_0.0.pkl, sensitivity_theta_0.02.pkl, ...
    sensitivity_theta = run_sensitivity_theta(
        lusail, model, D, norm_params, dist_gen, max_steps=500)

    # ================================================================
    # 步骤8：敏感性分析 - epsilon（论文表15）
    # ================================================================
    # 测试K-DRMPC在不同CVaR风险水平（epsilon）下的性能
    # epsilon控制风险约束的严格程度
    print("\n" + "#" * 60)
    print("# 敏感性分析 - EPSILON（表15）")
    print("#" * 60)
    # 在卢赛尔赛道上运行，测试EPSILON_VALUES中的所有epsilon值
    # 结果保存到：sensitivity_epsilon_0.01.pkl, sensitivity_epsilon_0.05.pkl, ...
    sensitivity_epsilon = run_sensitivity_epsilon(
        lusail, model, D, norm_params, dist_gen, max_steps=500)
